...
var copyFilesAndDirectories = async (sourceDir, targetDir) => {
...
var renamePackageJsonName = async (targetDir, projectName) => {
...
var updateAnchorToml = async (targetDir, packageManager) => {
  const anchorTomlPath = path.join(targetDir, "Anchor.toml");
  if (!fs.existsSync(anchorTomlPath)) {
    return;
  }
  let content = fs.readFileSync(anchorTomlPath, "utf-8");
  content = content.replace(/^package_manager\s*=\s*"[^"]*"/m, `package_manager = "${packageManager.name}"`);
  content = content.replace(/^test\s*=\s*"(npm run|yarn|pnpm|bun run)\s+ts-mocha/m, `test = "${packageManager.runCommand} ts-mocha`);
  ...
  try {
    const response = await import_prompts.default([
      ...
      },
      {
        ...
        validate: (val) => /^[a-z0-9-]+$/.test(val) ? true : "Project name should not contain special characters except hyphen (-)"
      }
    ]);
...
\uD83D\uDC4B Setup cancelled.`);
    ...
    const sourceDir = path2.resolve(fileURLToPath(import.meta.url), "../../templates", `${template}`);
